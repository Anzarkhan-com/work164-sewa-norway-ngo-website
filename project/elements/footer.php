<div class="w3l-footer-main" id="contact">
	<div class="w3l-sub-footer-content">
		<!-- Footers-14 -->
		<footer class="footer-14">
			<div id="footers14-block">
				<div class="container">
					<div class="footers20-content">
						<div class="d-grid grid-col-4 grids-content">
							<div class="columns">
								<h4>SEWA Norway</h4>
								<p>A ‘not-for-profit’ organization inspired by the noble
									tradition of service before self, and working towards
									promoting volunteerism</p>
							</div>
							<div class="columns">
								<h4>Reach Us</h4>
								<!-- <p><a href="tel:+44-000-888-999">+44-000-888-999</a></p><p style="font-size:12px;">(Mon-Fri 10:30-18:00)</p> -->
								<p> <a href="https://m.me/SewaNorway" target="_blank"> <img src="assets/images/messanger.png" class="img-fluid" style="max-width:40px;"> - Chat Now </a> <br> <br> </p>
								<p> Email - <a href="mailto:sewa@sewanorway.org">sewa@sewanorway.org</a></p>
							</div>
							<div class="columns">
								<h4>Inquiry Form</h4>
								<form action="inquiry.php" method="post" class="text-right">
									<div class="form-grid">
										<input type="text" name="name" id="w3lName" placeholder="Name*" required>
										<input type="email" name="email" id="w3lSender" placeholder="Email*" required>
										<input type="text" name="phone" id="w3lPhone" placeholder="Phone number*" required>
									</div>
									<textarea name="message" id="w3lMessage" placeholder="Message" style="margin-top:10px;"></textarea>
									<button type="submit" name="submit" class="btn btn-primary btn-style mt-3" style="padding:10px; line-height:18px; margin-right:20%;">Send
										Message</button>
								</form>
								<!-- <br> <p><a href="https://sites.google.com/view/sewanorway/home?_ga=2.76454611.737417139.1662107256-806751774.1660588655" target="_blank">Terms & Conditions</a></p>
                              <p><a href="#">Infopack</a></p>
                              <p><a href="#">Menu</a></p> -->
							</div>
							<div class="columns">
								<h4>Follow Us On</h4> <br>
								<ul class="d-flex">
									<!-- <li><a href="https://www.facebook.com/SewaNorway" target="_blank"><span class="fa fa-facebook" aria-hidden="true" style="color:#1877F2 !important;"></span></a></li> -->
									<!-- Start Facebook Link -->
									<li>
										<div class="search-right px-1">
											<a href="https://www.facebook.com/SewaNorway/" target="_blank"><span class="fa fa-facebook" aria-hidden="true" style="color:#1877F2 !important;"></span></a>
										</div>
									</li>
									<!-- End Facebook Link -->

									<!-- Start Instagram Link -->
									<li>
										<div class="search-right px-1">
											<a href="https://www.instagram.com/sewanorway/" target="_blank"><span class="fa fa-instagram" aria-hidden="true" style="color:#FF0098 !important;"></span></a>
										</div>
									</li>
									<!-- End Instagram Link -->

									<!-- Start Linkedin Link -->
									<div class="search-right px-1">
										<a href="#" target="_blank"><span class="fa fa-linkedin" aria-hidden="true" style="color:#0A66C2 !important;"></span></a>
									</div>
									<!-- End Linkedin Link -->

									<!-- Start Twitter Link -->
									<li>
										<div class="search-right px-1">
											<a href="#" target="_blank"><span class="fa fa-twitter" aria-hidden="true" style="color:#1DA1F2 !important;"></span></a>
										</div>
									</li>
									<!-- End Twitter Link -->

									<!-- Start Youtube Link -->
									<li>
										<div class="search-right px-1">
											<a href="https://www.youtube.com/@sewanorway2102" target="_blank"><span class="fa fa-youtube" aria-hidden="true" style="color:#FF0000 !important;"></span></a>
										</div>
									</li>
									<!-- End Youtube Link -->
							</div>
						</div>
					</div>
					<div class="footers14-bottom d-flex">
						<div class="copyright">
							<p>© 2022 SEWA Norway. All rights reserved | <a href="terms.php" target="_blank">Terms & Conditions</a>
							</p>
							<p style="color:#202020"> Design by <a href="https://w3layouts.com/" target="_blank" style="color:#202020 !important;">W3Layouts</a></p>
						</div>
						<div class="language-select d-flex">
							<!-- <span class="fa fa-language" aria-hidden="true"></span> -->
							Maintained By :<a href="https://socialtalkers.in" target="_blank" style="margin-left: 8px;"> Social Talkers </a>
							<!-- <select>
                              <option>English</option>
                              <option>Estonina</option>
                              <option>Deutsch</option>
                              <option>Nederlan;ds</option>
                          </select> -->
						</div>
					</div>
				</div>
			</div>
			<!-- move top -->
			<button onclick="topFunction()" id="movetop" title="Go to top">
				&uarr;
			</button>
			<script>
				// When the user scrolls down 20px from the top of the document, show the button
				window.onscroll = function() {
					scrollFunction()
				};

				function scrollFunction() {
					if (document.body.scrollTop > 20 || document.documentElement.scrollTop > 20) {
						document.getElementById("movetop").style.display = "block";
					} else {
						document.getElementById("movetop").style.display = "none";
					}
				}
				// When the user clicks on the button, scroll to the top of the document
				function topFunction() {
					document.body.scrollTop = 0;
					document.documentElement.scrollTop = 0;
				}
			</script>
			<!-- /move top -->
		</footer>
		<!-- Footers-14 -->
	</div>
</div>